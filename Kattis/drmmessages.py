...
def solve():
    ...
    message = input()
    a = message[:int(len(message) / 2)]
    b = message[int(len(message) / 2):]
    ...
    for x in range(len(a)):
        sum1 = sum1 + alphabet.index(a[x])
        sum2 = sum2 + alphabet.index(b[x])

    result = ""

    for x in range(len(a)):
        result = result + alphabet[(alphabet.index(a[x]) + sum1 + ((alphabet.index(b[x]) + sum2) % 26)) % 26]
    print(result)
